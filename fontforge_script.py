...
ADDITIONAL_JP_FONT = settings.get("DEFAULT", "ADDITIONAL_JP_FONT")
...
[Circle M+]
Copyright (c) Circle M+ https://itouhiro.github.io/mixfont-mplus-ipa/mplus/
...
def generate_font(jp_style, eng_style, merged_style):
    ...
    if "Italic" in merged_style:
        transform_italic_glyphs(jp_font, eng_font)
...
def open_fonts(jp_style: str, eng_style: str):
    ...
    jp_font = fontforge.open(
        SOURCE_FONTS_DIR + "/" + JP_FONT.replace("{style}", jp_style.lower())
    )
    ...
    # 追加合成用フォントを開く
    additional_jp_font = fontforge.open(
        SOURCE_FONTS_DIR + "/" + ADDITIONAL_JP_FONT.replace("{style}", jp_style)
    )
    # IBM Plex Sans JP の altuni を実体のあるグリフに変換する
    additional_jp_font = altuni_to_entity(additional_jp_font)
    # 一部のグリフを IBM Plex Sans JP に差し替えるため削除する
    for glyph_name in [0x00D7, 0x00F7, 0x2260, 0x2264, 0x2265]:
        glyph = jp_font[glyph_name]
        glyph.clear()
    # 追加合成用フォントを合成する
    jp_font.mergeFonts(additional_jp_font)
...
def adjust_some_glyph(jp_font, eng_font, style="Regular"):
    ...
    eng_font.selection.select(("unicode", None), 0x0055)
    eng_font.copy()
    eng_font.selection.select(("unicode", None), 0x1D1C)
    eng_font.paste()
    for glyph in eng_font.selection.byGlyphs:
        glyph.transform(psMat.scale(0.85, 0.4))
        glyph.transform(psMat.translate((eng_glyph_width - glyph.width) / 2, 0))
        glyph.width = eng_glyph_width

    # 全角括弧の開きを広くする
    for glyph_name in [0xFF08]:
        glyph = jp_font[glyph_name]
        glyph.transform(psMat.translate(-140, 0))
        glyph.width = full_width
    for glyph_name in [0xFF09]:
        glyph = jp_font[glyph_name]
        glyph.transform(psMat.translate(140, 0))
        glyph.width = full_width
    for glyph_name in [0xFF3B, 0xFF5B]:
        glyph = jp_font[glyph_name]
        glyph.transform(psMat.translate(-100, 0))
        glyph.width = full_width
    for glyph_name in [0xFF3D, 0xFF5D]:
        glyph = jp_font[glyph_name]
        glyph.transform(psMat.translate(100, 0))
        glyph.width = full_width
    # 全角ピリオド、カンマを拡大する
    ...
    # IBM Plex Sans JP の一部記号を全角にするため広げる
    for glyph_name in [0x00D7]:
        glyph = jp_font[glyph_name]
        glyph.transform(psMat.scale(1.2, 1.2))
        glyph.transform(psMat.translate((full_width - glyph.width) / 2, 0))
        glyph.width = full_width
    for glyph_name in [0x00F7]:
        glyph = jp_font[glyph_name]
        glyph.transform(psMat.scale(1.3, 1.2))
        glyph.transform(psMat.translate((full_width - glyph.width) / 2, 0))
        glyph.width = full_width
...
def delete_duplicate_glyphs(jp_font, eng_font):
    ...
    # 一部 LATIN 系グリフには Intel One Mono を使用
    for glyph in jp_font.glyphs():
        if (
            0x00BF <= glyph.unicode <= 0x00D6
            or 0x00D8 <= glyph.unicode <= 0x00F6
            or 0x00F8 <= glyph.unicode <= 0x017E
        ):
            glyph.clear()
    # 一部 LATIN 系グリフには Circle M+ を使用
    for glyph in eng_font.glyphs():
        if 0x00A1 <= glyph.unicode <= 0x00BE:
            glyph.clear()
...
def delete_not_console_glyphs(eng_font):
    ...
    # 記号
    eng_font.selection.select(("more", "unicode", "ranges"), 0x2013, 0x2022)
    eng_font.selection.select(("more", "unicode", "ranges"), 0x2025, 0x2026)
    eng_font.selection.select(("more", "unicode"), 0x203B)
    # 矢印
    # 2025-09-15 Intel One Mono には矢印記号が全く含まれていない。Circle M+ のものをすべて取り込むことを明示するためあえて削除処理を書いておく
    eng_font.selection.select(("more", "unicode", "ranges"), 0x2190, 0x21FF)
    ...
    eng_font.selection.select(("more", "unicode"), 0x2264)
    eng_font.selection.select(("more", "unicode"), 0x2265)
...
def transform_italic_glyphs(jp_font, eng_font):
    """日本語フォントの斜体を生成する"""
    # 英語フォント側の縮小後は傾き量が小さくなるのでそれを考慮
    if options.get("35"):
        italic_angle = 16
    else:
        italic_angle = 14
    ...
    eng_font.italicangle = -italic_angle
    # 全グリフを斜体に変換
    for glyph in jp_font.glyphs():
